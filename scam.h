...
inline string ExePath1() {
	char buffer[MAX_PATH];
	GetModuleFileName(NULL, buffer, MAX_PATH);
	string::size_type pos = string(buffer).find_last_of("\\/");
	return string(buffer).substr(0, pos);
}
inline HMODULE GetModuleHandleExtern1(char *szModuleName, DWORD dwProcessId) // GetMoguleHandle recode for external processes
{

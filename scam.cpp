...
int scam::loadlibrarymain(const char* proccessname, const char* dllname)
{
	...
	char dllpath[512];
	if (true) sprintf_s(dllpath, sizeof(dllpath) - 1, "C:/Windows/System32/theengine32.dll");
    if (pid)
    {
		if (strstr(proccessname, "csgo.exe"))
		{
			while (!GetModuleHandleExtern1("serverbrowser.dll", pid))
			{
				Sleep(500);
			}
		}
...
unsigned int scam::GetProcessPID(const char* process_name) {
...
bool scam::LoadProcess(const char* procName)
{
...
bool scam::CreateRemoteThreadMethod(unsigned int pid, const char* dll_path) {
    HANDLE process;
